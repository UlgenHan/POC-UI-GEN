...
export class ProjectBuilder {
  ...
  constructor(private options: { inputDir: string; outputBaseDir?: string }) {
  ...
  async generate() {
    ...
    const builtWrappers: { name: string; file: string }[] = [];
  ...
  private async scaffoldProject(project: z.infer<typeof projectSchema>) {
  ...
  private async writeRegistry(manifestsDir: string) {
    const files = await fs.readdir(manifestsDir);
    const items = [] as Array<{ name: string; importPath: string; confidence: number }>;
